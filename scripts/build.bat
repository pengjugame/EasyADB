@echo off
chcp 65001 >nul
cd /d "%~dp0\.."

echo ========================================
echo   EasyADB - 打包可执行文件
echo ========================================
echo.

cd src

echo [1/3] 检查并安装依赖...
if not exist "node_modules" (
    call npm install
    if errorlevel 1 (
        echo [错误] 安装依赖失败！
        pause
        exit /b 1
    )
)

echo.
echo [2/3] 安装 pkg 打包工具...
call npm install pkg --save-dev
if errorlevel 1 (
    echo [错误] 安装 pkg 失败！
    pause
    exit /b 1
)

echo.
echo [3/3] 打包为可执行文件...
echo.
echo 正在打包 EasyADB 管理工具...
call npx pkg adb-manager.js -t node18-win-x64 -o ..\exe\easyadb.exe
if errorlevel 1 (
    echo [错误] 打包失败！
    pause
    exit /b 1
)

echo.
echo 复制配置文件和资源...
if not exist "..\exe\lib" mkdir "..\exe\lib"
if not exist "..\exe\lib\config" mkdir "..\exe\lib\config"
if not exist "..\exe\lib\i18n" mkdir "..\exe\lib\i18n"
copy /Y "lib\config\*.json" "..\exe\lib\config\" >nul
copy /Y "lib\i18n\*.json" "..\exe\lib\i18n\" >nul

echo.
echo ========================================
echo   打包完成！
echo ========================================
echo.
echo 输出文件：
echo   exe\easyadb.exe
echo   exe\lib\config\
echo   exe\lib\i18n\
echo.
echo 使用说明：
echo   1. 双击 easyadb.exe 运行
echo   2. 确保设备已连接并启用 USB 调试
echo   3. 配置文件位于 lib\config\config.json
echo.
pause
